...
import { RouterUtil, SelectorUtil } from '@jc4f-nx/spotify-shared-utils';
...
export class AlbumStore extends ComponentStore<AlbumState> {
  ...
  isAlbumPlaying$ = SelectorUtil.getMediaPlayingState(
    combineLatest([
      this.album$.pipe(map((album) => album?.uri)),
  ...
  readonly toggleAlbum = this.effect<{ isPlaying: boolean; uri: string }>(
    (params$) =>
      params$.pipe(
