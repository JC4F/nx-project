...
import { TrackApiService } from '@jc4f-nx/spotify-shared-data-access-spotify-api';
import { RouteUtil, StringUtil } from '@jc4f-nx/spotify-shared-utils';
...
export class PlaybackStore extends ComponentStore<PlaybackState> {
  ...
  readonly currentTrack$: Observable<SpotifyTrackExtended | null> =
    this.playback$.pipe(
      filter((data) => !!data),
      map(({ context, track_window }) => {
        ...
        const getPlaylistUrl = (uri: string | null) => {
          ...
          return isPlaylist
            ? RouteUtil.getPlaylistRouteUrl(StringUtil.getIdFromUri(uri))
            : '';
        };
